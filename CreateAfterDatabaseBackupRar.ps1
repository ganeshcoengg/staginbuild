...
Write-Host "Creating database RAR for after deployment..."
LogWrite "Creating database RAR for after deployment..."
try{
    ...
    $FileName="mysqldumpIndiDB_AFTER_$filedate"
    ...
    if($dirinfo.Length -eq 0){
        ...
        if($?){
            Write-Host "Database RAR for after deployment has created!"
            LogWrite "Database RAR for after deployment has created!"
        }
        else{
            $ErrorLog = $Error[0].Exception.Message 
            Write-Host "Database RAR for after deployment has failed" -ForegroundColor Red
            LogWrite "Database RAR for after deployment has  failed $ErrorLog" $true
        }
    }
}
